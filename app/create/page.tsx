...
export default function CreatePromptPage() {
	...
	const handleModelChange = (modelId: string) => {
		if (modelId !== "models/gemini-2.0-flash-exp") {
			setShowModelAlert(true);
			setSelectedModelId("models/gemini-2.0-flash-exp");
		} else {
	...
	const handleToggleStatus = async (
		...
		try {
			...
			toast({
				...
				description: `Prompt đã được chuyển sang ${
					newStatus === "public" ? "công khai" : "riêng tư"
				}`,
			});
		} catch (error) {
							...
							<div className="border rounded-lg min-h-[calc(100vh-400px)]">
								{result || isProcessing ? (
									<Tabs
										...
										value={activeTab}
										onValueChange={(value) =>
											setActiveTab(value as "thinking" | "prompt")
										}
										className="h-full">
